...
import 'package:purificadora_app/data/services/geocoding_service.dart';
...
Future<void> _crearPedido(double total) async {
  ...
  try {
    ...
    final coords = await geocoding.getCoordinates(direccion);

    if (coords == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("No se pudo ubicar la dirección")),
      );
      return;
    }

    /// 🔥 2. CREAR PEDIDO (CORRECTO)
    await _clientService.crearPedido(
      ...
      cantidad: quantity,
      total: total,
      ubicacion: GeoPoint(coords.latitude, coords.longitude),
    );
